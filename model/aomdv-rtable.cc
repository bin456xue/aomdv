...
RoutingTableEntry::Path::Print (Ptr<OutputStreamWrapper> stream) const
{
  ...
  *os << std::setiosflags (std::ios::fixed) << 
  ...
  //std::setw (14) <<(uint32_t)( (m_expire - Simulator::Now ()) < Seconds(0) );
  *os << "\t" << m_hopCount<<"\n";
}
                               ...
                               Time expireTime, Ipv4Address lastHop, Ipv4InterfaceAddress iface)
{
  ...
  //NS_LOG_INFO("nextHop: "<<nextHop);
  m_pathList.push_back (path);
  m_numPaths += 1;     //TODO
  Path *p =  &(m_pathList[m_pathList.size() - 1]); //new Path(dev, m_dst, nextHop, hopCount, expireTime, lastHop, iface);
  return p;
}
...
RoutingTableEntry::PathDelete (Ipv4Address id)
{
  NS_LOG_FUNCTION (this << id);
  PathPurge();
  for (std::vector<Path>::iterator i = m_pathList.begin (); i!= m_pathList.end (); )
  {
	NS_ASSERT(i->m_ipv4Route!=NULL);
    if (i->m_ipv4Route->GetGateway () == id)
    {
      NS_LOG_LOGIC ("Path " << id << " found");
      std::vector<Path>::iterator tmp = i;
      //i++;
      i = m_pathList.erase (tmp);
      m_numPaths -= 1;   //TODO
      //break;
    }
    else
    ...
    NS_LOG_INFO("path nums: "<<m_numPaths<<" length: "<<m_pathList.size());
  }
}
...
{
  for (std::vector<Path>::iterator j = m_pathList.begin (); j!= m_pathList.end (); )
    {
      if (j->GetInterface () == iface)
        {
          std::vector<Path>::iterator tmp = j;
          //j++;
          j = m_pathList.erase (tmp);
...
RoutingTableEntry::PathFind (void) 
{
  ...
  //NS_LOG_INFO("size: "<<m_pathList.size());
  return &(*i);
  //path =
  //return path;
}
...
void
RoutingTableEntry::RtDown()
{
	if(GetFlag() == INVALID)
		return;
	m_flag = INVALID;
	m_advertisedHopCount = INFINITY2;
	PathAllDelete();
...
RoutingTable::LookupRoute (Ipv4Address id )
{
  ...
  return const_cast<RoutingTableEntry*>(&(i->second));
}
...
bool
RoutingTable::rt_has_active_route()
{
	Purge ();
	if(m_ipv4AddressEntry.size() > 0)
		return true;
	return false;
}
bool
...
RoutingTable::AddRoute (RoutingTableEntry & rt)
{
  ...
  //map里面单独修改value的值，不修改key值，插入不会成功
          ...
          if (i->second.GetFlag () == INVALID)
            {
              std::map<Ipv4Address, RoutingTableEntry>::iterator tmp = i;
              //++i;
              i = m_ipv4AddressEntry.erase (tmp);
            }
          else if (i->second.GetFlag () == VALID)
            {

        	    std::vector<RoutingTableEntry::Path>::iterator j;
                for(j=i->second.m_pathList.begin(); j!=i->second.m_pathList.end(); ){
                	if(j->GetExpire() < Seconds (0)){
                		std::vector<RoutingTableEntry::Path>::iterator tempj = j;
                		//j++;
                		j = i->second.m_pathList.erase(tempj);
                		i->second.m_numPaths = i->second.m_numPaths-1;
                		if(i->second.m_numPaths  == 0)
                		{
                			break;
                		}
                		//NS_LOG_INFO("m_numPaths: "<<i->second.m_numPaths);
                		//NS_ASSERT(i->second.m_numPaths>=0);
...
{
  NS_LOG_INFO("yyyyyyyy");
  NS_LOG_FUNCTION (this);
  if (table.empty ())
    return;
  NS_LOG_INFO("zzzzzzzzzzzz");
  for (std::map<Ipv4Address, RoutingTableEntry>::iterator i =
         table.begin (); i != table.end ();)
    {
	  //删除采用的策略：等待过期再进行删除操作。不可以用的路由条目迟早会过期
      if ( i->second.GetLifeTime () < Seconds (0) )
        {
    	  NS_LOG_INFO("tttttttttttttt");
          if (i->second.GetFlag () == INVALID)
            {
              std::map<Ipv4Address, RoutingTableEntry>::iterator tmp = i;
              i = table.erase (tmp);
            ...
            {
      	       std::vector<RoutingTableEntry::Path>::iterator j;
      	       NS_LOG_INFO("xxxxxxxxxxxxx");
               //std::cout<<"val: "<<(uint32_t)(i->second.m_pathList.size() == (uint32_t)i->second.m_numPaths)<<std::endl;
               for(j=i->second.m_pathList.begin(); j!=i->second.m_pathList.end(); )
               {
